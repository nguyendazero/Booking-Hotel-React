...
const AmenityModal = ({
  ...
  const { postData: addAmenity, loading: addLoading, error: addError } = usePost(
    "http://localhost:8080/api/v1/owner/hotel-amenity"
  );
  const { deleteData: deleteAmenity, loading: deleteLoading, error: deleteError } = useDelete();
  ...
  return (
    <Modal
      ...
      onCancel={onClose}
      footer={[
        <Button key="cancel" onClick={onClose} disabled={addLoading || deleteLoading}>
          Cancel
        </Button>,
      ]}
    >
      <div>
        ...
        {(addLoading || deleteLoading) && <LoadingSpinner text={addLoading ? "Adding amenity..." : "Deleting amenity..."} />}
      </div>
    </Modal>
  );
};
